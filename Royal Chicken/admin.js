...
/* =========================================================
   PRINT INVOICE - PREMIUM PDF / PRINT
========================================================= */
        ...
        <head>
            ...
            <style>
                ...
                * {
                ...
                .invoice-footer-note span:first-child {
...
window.sendInvoiceWhatsApp = function(orderId) {
    ...
    let message =
        ...
        `📅 *Date:* ${formatDate(getDateValue(order.createdAt))}%0A` +
        `━━━━━━━━━━━━━━━━━━%0A`;
    items.forEach((item, index) => {
        message +=
            `${index + 1}. ${item.name || "-"} ` +
            `× ${item.quantity || 0} ` +
            `= ${money(item.itemTotal)}%0A`;
    });
    message +=
        `━━━━━━━━━━━━━━━━━━%0A` +
        `💰 *TOTAL: ${money(order.total)}*%0A%0A` +
        `Thank you for ordering from *Royal Chicken*! 🍗`;
    let whatsappUrl;
    if (phone.length >= 10) {
        const indianPhone =
            phone.length === 10
                ...
                : phone;
        whatsappUrl =
            `https://wa.me/${indianPhone}?text=${message}`;
    } else {
        whatsappUrl =
